...
def get_tracer(name: str = "toolbox", version: Optional[str] = None) -> Tracer:
    """Get or create a tracer for MCP operations.

    Args:
        name: The tracer name
        version: The tracer version
...
def get_meter(name: str = "toolbox-core-mcp", version: Optional[str] = None) -> Meter:
    """Get or create a meter for MCP metrics.

    Args:
        name: The meter name
        version: The meter version
...
def create_operation_duration_histogram(meter: Meter) -> Optional[Histogram]:
    ...
    try:
        return meter.create_histogram(
            ...
            # Note: explicit_bucket_boundaries parameter may not be available in all versions
            # If not available, the SDK will use default buckets
        )
    except Exception:
        return None
...
def start_span(
    ...
    """Start a telemetry span for MCP operations. Returns None if telemetry fails.
    ...
    Returns:
        The started span, or None if telemetry failed
    """
    try:
...
def end_span(span: Optional[trace.Span], error: Optional[Exception] = None) -> None:
...
def record_error_from_jsonrpc(span: trace.Span, error_code: int, error_message: str) -> None:
...
def setup_otlp_tracer_provider(
    otlp_endpoint: str, service_name: str = "toolbox-core-mcp"
) -> None:
    """Setup OTLP tracer provider to send telemetry to an OTLP endpoint.

    Args:
        otlp_endpoint: The OTLP endpoint URL (e.g., "http://localhost:4318")
        service_name: The service name for the resource
    """
    # Create resource with service name
    resource = Resource.create({"service.name": service_name})

    # Create OTLP exporter with the provided endpoint
    # The endpoint should include the full path for HTTP: http://host:port/v1/traces
    # If only host:port is provided, append the traces path
